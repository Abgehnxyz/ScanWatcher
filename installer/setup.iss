...
AppPublisherURL=https://www.nova-network.one
...
Filename: "powershell.exe"; \
  Parameters: "-NonInteractive -WindowStyle Hidden -Command ""$cfg = try {{ Get-Content -Raw -Path ([IO.Path]::Combine($env:APPDATA,'ScanWatcher','config.json')) | ConvertFrom-Json }} catch {{ $null }}; if ($cfg.telemetry_enabled -eq $true) {{ $id = Get-Content -ErrorAction SilentlyContinue -Path ([IO.Path]::Combine($env:APPDATA,'ScanWatcher','install_id')); if ($id) {{ Invoke-RestMethod -Method Post -Uri 'https://ops.abgehn.xyz/api/scanwatcher/event' -ContentType 'application/json' -Body ('{{""install_id"":""' + $id.Trim() + '"",""event"":""uninstall""}}') -ErrorAction SilentlyContinue }} }}"""; \
  Flags: runhidden
