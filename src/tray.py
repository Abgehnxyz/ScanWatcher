...
class TrayApp:
    ...
    def _open_settings(self, icon=None, item=None):
        # Wenn Hauptfenster offen: SettingsDialog im selben Tk-Thread oeffnen.
        # Zwei parallele Tk-Instanzen (Hauptfenster + eigener Thread) blockieren
        # auf Windows UI-Events wie Folder-Picker und Speichern-Klick.
        if self._main_window is not None and self._main_window is not True:
            try:
                self._main_window.after(
                    0,
                    lambda: SettingsDialog(
                        self._main_window, self.cfg,
        ...
        # Kein Hauptfenster offen: eigenen Thread + Root starten
        def run_dialog():
    ...
    def _show_main_window(self, icon=None, item=None):
        # Nur ein Fenster gleichzeitig – auch gegen schnelle Doppelklicks
        ...
        self._main_window = True  # Sentinel – blockiert weitere Aufrufe bis Fenster bereit
